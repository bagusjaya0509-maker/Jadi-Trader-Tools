...
function Baris({ r, aku, onPilih }: {
  ...
  return (
    <Bungkus
      ...
      <span className="angka w-5 shrink-0 text-center text-[12px] text-zinc-200">{r.rank}</span>
      {r.rank <= 3
      ...
      <span className="min-w-0 grow">
        <span className="flex items-center gap-1.5">
          <span className="truncate text-[12.5px] font-medium text-white [text-shadow:0_1px_3px_rgb(0_0_0/0.45)]">{r.userName}</span>
        ...
        {(r.byline || r.ekstra) && (
          <span className="block text-[10.5px] leading-snug text-zinc-200 line-clamp-2">
            {r.byline}
            {r.ekstra && (
      ...
      <span className="shrink-0 text-[9.5px] uppercase tracking-wide text-zinc-200">PNL</span>
